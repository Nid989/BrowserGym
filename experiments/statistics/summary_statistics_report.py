...
def parse_experiment_metadata(exp_dir: Path) -> Dict:
    ...
    match = re.match(
        r"(\d{4}-\d{2}-\d{2})_(\d{2}-\d{2}-\d{2})_DemoAgentArgs_on_workarena.servicenow.(.*)_(\d+)",
        experiment_name,
    )
    if not match:
        return {}
...
def save_summary(summary: Dict, results_dir: Path):
    last_experiment_time = max(
        exp_dir.name
        for exp_dir in results_dir.iterdir()
        if re.match(r"\d{4}-\d{2}-\d{2}_\d{2}-\d{2}-\d{2}", exp_dir.name)
    )
    timestamp = (
        last_experiment_time.replace("_", " ").replace("-", ":", 1).replace("-", ":", 1)
    )
    markdown_file = (
        results_dir
        / f"{timestamp[0:4]}-{timestamp[5:7]}-{timestamp[8:10]}_statistics.md"
    )
    csv_file = (
        results_dir
        / f"{timestamp[0:4]}-{timestamp[5:7]}-{timestamp[8:10]}_statistics.csv"
    )
    ...
    with open(markdown_file, "w") as md:
        ...
        experiment_rows = []
        for experiment, stats in sorted(
            summary["experiments"].items(), key=lambda item: item[1]["date_time"]
        ):
        ...
        overall_rows = [
            [
                "Runs",
                format_number(summary["total_runs"]),
                format_number(summary["successful_runs"]),
                format_number(summary["failed_runs"]),
            ],
            [
                "Steps",
                format_number(summary["steps"]["total"]),
                format_number(summary["steps"]["successful"]),
                format_number(summary["steps"]["failed"]),
            ],
            [
                "Tokens",
                format_number(summary["tokens"]["total"]),
                format_number(summary["tokens"]["successful"]),
                format_number(summary["tokens"]["failed"]),
            ],
            [
                "Elapsed Time (s)",
                format_number(summary["elapsed_time"]["total"]),
                format_number(summary["elapsed_time"]["successful"]),
                format_number(summary["elapsed_time"]["failed"]),
            ],
            [
                "Error Logs",
                format_number(summary["error_logs"]["total"]),
                format_number(summary["error_logs"]["successful"]),
                format_number(summary["error_logs"]["failed"]),
            ],
        ]
        write_section(
            md,
        ...
        task_overview_rows = [
            [
                task,
                format_number(stats["total_runs"]),
                format_number(stats["successful_runs"]),
                format_number(stats["failed_runs"]),
            ]
        ...
        for task, stats in summary["tasks"].items():
            task_rows = [
                ...
                [
                    "Steps",
                    format_number(stats["steps"]["total"]),
                    format_number(stats["steps"]["successful"]),
                    format_number(stats["steps"]["failed"]),
                ],
                [
                    "Tokens",
                    format_number(stats["tokens"]["total"]),
                    format_number(stats["tokens"]["successful"]),
                    format_number(stats["tokens"]["failed"]),
                ],
                [
                    "Elapsed Time (s)",
                    format_number(stats["elapsed_time"]["total"]),
                    format_number(stats["elapsed_time"]["successful"]),
                    format_number(stats["elapsed_time"]["failed"]),
                ],
                [
                    "Error Logs",
                    format_number(stats["error_logs"]["total"]),
                    format_number(stats["error_logs"]["successful"]),
                    format_number(stats["error_logs"]["failed"]),
                ],
            ]
            write_section(
                md,
...
def main():
    parser = argparse.ArgumentParser(
        description="Analyze experiment results and generate a summary report."
    )
    parser.add_argument(
        "--results_dir",
        type=str,
        nargs="?",
        default=".",
        help="Path to the results directory containing experiment subdirectories.",
    )
    parser.add_argument(
        "--debug",
        action="store_true",
        help="Enable debug mode logging for more detailed output.",
    )
