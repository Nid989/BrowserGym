                ...
                if path:
                    ...
                    for i, node in enumerate(path):
                        ...
                        formatted_text.append(
                            f"{indent}[{node['bid']}] {node['role']}: '{node['name']}'\n"
                        )
                    formatted_text.append("```\n\n")
                else:
...
class PastActionTracker:
    def __init__(self, use_thought: bool = False):
        ...
        self.axtrees: Dict[
            int, str
        ] = {}  # Dictionary to store accessibility trees for each step
        ...
        with open(f"{exp_dir}/experiment_analysis.md", "w") as f:
            ...
            if steps_info and steps_info[0].obs and "goal_object" in steps_info[0].obs:
                ...
                if goal_texts:
                    ...
                    f.write(
                        "* **Derived Goal**: No text goals found in goal_object\n\n"
                    )
            else:
            ...
            f.write(
                f"* **Date of Instance Run**: {exp_date.strftime('%Y-%m-%d %H:%M:%S')}\n"
            )
            ...
            for step_info in steps_info:
                ...
                if step_info.obs:
                    for idx, (url, title) in enumerate(
                        ...
                        active = (
                            "(active)"
                            if idx == step_info.obs["active_page_index"]
                            else ""
                        )
            ...
            f.write(
                get_system_message_template(
                    chat_mode=False, system_message=system_message
                )
            )
            f.write("\n```\n\n")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Analyze BrowserGym experiment results"
    )
    parser.add_argument(
        "--exp_dir", type=str, help="Path to single experiment directory"
    )
    parser.add_argument(
        "--results_dir",
        type=str,
        default="./result",
        help="Path to results directory containing multiple experiments",
    )
    parser.add_argument(
        "--debug", action="store_true", help="Enable debug mode logging"
    )
    parser.add_argument(
        "--system_message", type=str, help="Custom system message to use in templates"
    )
    parser.add_argument(
        "--use_thought",
        action="store_true",
        default=False,
        help="Include thought process in the output",
    )
